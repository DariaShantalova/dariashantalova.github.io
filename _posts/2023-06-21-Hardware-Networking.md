...
### Segregated Buffer
Пример: Broadcom Trident и Tomahawk
группировка портов по 4-8 
каждой группе - кусок общего буфера
группа не может лезть в чужие куски
порт берет память только из своего куска

### Admission control
функция - можно ли записывать пакет в буфер
виды
...
* Egress - регуляция вспелсков - чтобы несколько портов не заполнили весь буфер
  объем памяти выделяемой на порт определяется порогом (статическим или динамическим alpha)
...
```
пример 32 порта
alpha = 1/64, при максимальной загрузке только половина памяти
1/32 - вся
1/16 - порт может получить большую часть

### Output Queuing
расположение буфероф перед выходными интрефейсами
приходят на Egress PFE чтобы умирать
(-) бесполезная загрузка фабрики коммутации

<img width="591" alt="Screenshot 2023-07-01 at 18 28 46" src="https://github.com/DariaShantalova/dariashantalova.github.io/assets/34622678/9460958f-0019-47b4-87c4-09dbfa34d9fd">


### Input Queuing
буферы после лукапа, после того как становится понятно куда посылать пакет
(-) а если выходной порт занят, придется подождать


<img width="591" alt="Screenshot 2023-07-01 at 18 42 11" src="https://github.com/DariaShantalova/dariashantalova.github.io/assets/34622678/a2991670-2214-47ee-a6c5-f9270a988528">
...
арбитр проверяет загруженность выходного буфера а не интерфеса
минусы
* два буфера -> увеличивается цена
* увеличивается задержка
* QoS в двух местах
  
<img width="531" alt="Screenshot 2023-07-01 at 18 56 47" src="https://github.com/DariaShantalova/dariashantalova.github.io/assets/34622678/a17a2857-aa47-4999-91a9-4a8b9f41a719">
...
в IP пакете - 8 бит под QOS
6 бит под DSCP, 2 бита под ECN
принцип: транзитное устройство перед перегрузкой отправляет ECN отправителю
